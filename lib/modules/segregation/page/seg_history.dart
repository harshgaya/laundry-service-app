...
import 'package:laundry_service/modules/segregation/page/segregation_remarks_page.dart';
...
class _SegHistoryState extends State<SegHistory> {
  ...
  void initState() {
    ...
    super.initState();
    segController.loadActiveInactive();
  }
    ...
    return Column(
      children: [
        Obx(() => HomeTaskNumberPage(
              todo: segController.activeCollection.value?.data
                      .where(
                          (element) => element.currentStatus == 'DRYING_DONE')
                      .length ??
                  0,
              open: segController.activeCollection.value?.data
                      .where((element) =>
                          element.currentStatus == 'IN_SEGREGATION')
                      .length ??
                  0,
              finished: segController.activeCollection.value?.data
                      .where((element) =>
                          element.currentStatus == 'SEGREGATION_DONE')
                      .length ??
                  0,
              overdue: 0,
              title: 'History',
        ...
        Obx(() => Expanded(
            child: segController.gettingActiveCollection.value
                ? Center(
                ...
                : segController.activeCollection.value == null
                    ? const SizedBox()
                    : ListView.builder(
                        padding: EdgeInsets.zero,
                        itemCount:
                            segController.activeCollection.value!.data.length,
                        itemBuilder: (context, index) {
                          final item =
                              segController.activeCollection.value!.data[index];
                          ...
                          if (isWithinDateRange) {
                            return TaskTileWidget(
                              ...
                              function: () {
                                ...
                                //           'IN_SEGREGATION', item.statusEntry),
                                //       segCompleted: Utils.getTimeForStatus(
                                //           'SEGREGATION_DONE', item.statusEntry),
